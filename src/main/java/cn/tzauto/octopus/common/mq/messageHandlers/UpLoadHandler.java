...
//    private static Logger logger = LoggerFactory.getLogger(UpLoadHandler.class);
    ...
    public void handle(Message message) {
        ...
        try {
            ...
            String rcpBasePath = mapMessage.getString("rcpBasePath");
            String attachType = mapMessage.getString("lastname");
            logger.info("接收到服务端rcpBasePath:" + rcpBasePath);
            logger.info("接收到服务端attachType:" + attachType);
           UiLogUtil.getInstance().appendLog2SeverTab(deviceCode, "服务端请求上传Recipe:[" + recipeName + "]");
            ...
            if (recipeMap != null) {
                ...
                recipeParaList = recipeService.saveUpLoadRcpInfo(rcpBasePath, recipe, recipeParaList);
            ...
            mqMap.put("resultDesc", JSONArray.toJSONString(recipeMap.get("resultDesc") == null ? "" : recipeMap.get("")));
