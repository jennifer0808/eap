...
public class BackUpRecipeHandler implements MessageHandler {
    ...
    private List<RecipePara> getMonitorRecipeParaList(List<RecipeTemplate> recipeTemplates, List<RecipePara> recipeParas) {
        if (recipeTemplates != null && !recipeTemplates.isEmpty()) {
            List<RecipePara> recipeParasTemp = new ArrayList<>();
            for (RecipePara recipePara : recipeParas) {
                for (RecipeTemplate recipeTemplate : recipeTemplates) {
                    if (recipeTemplate.getParaCode().equals(recipePara.getParaCode())) {
                        recipeParasTemp.add(recipePara);
                    }
                }
            }
            return recipeParasTemp;
        }
        return recipeParas;
    ...
    public void handle(HashMap<String, String> msgMap) throws IOException, HsmsProtocolNotSelectedException, T6TimeOutException, BrokenProtocolException, T3TimeOutException, ItemIntegrityException, StreamFunctionNotSupportException, MessageDataException, InterruptedException {
        ...
        recipeList = (List<Recipe>) JsonMapper.String2List(msgMap.get("recipeList"), Recipe.class);
        recipeParaList = (List<RecipePara>) JsonMapper.String2List(msgMap.get("recipeParaList"), RecipePara.class);
        attachs = (List<Attach>) JsonMapper.String2List(msgMap.get("attachs"), Attach.class);
        ...
        mqMap.put("eventName", "recipeBackup结果");
        try {
            ...
            GlobalConstants.C2SSpecificDataQueue.sendMessage(backupreusltMap);
        } catch (Exception e) {
            sqlSession.rollback();
