 ...
 */
package cn.tzauto.octopus.common.resolver.icos;
...
public class TrT830RecipeUtil {
    ...
    private static final Logger logger = Logger.getLogger(TrT830RecipeUtil.class);
...
//                    }
                    //只解析这三个文件来获取数据
                    if (fileName.contains(".han") || fileName.contains(".BGA_1") || fileName.contains(".CMO_1") || fileName.contains(".LLI")) {
                        ...
                        while ((lineString = br.readLine()) != null) {
                            ...
                            } else if (fileName.contains(".BGA_1")) {
                                list.add(lineString);
    ...
    public static void readLine(List<String> list, String fileName) {
        ...
        if (list != null && !list.isEmpty()) {
            for (int j = 0; j < list.size(); j++) {
                if (fileName.contains(".BGA_1") || fileName.contains(".LLI")) {
...
//            list.add(rcpName);
            while ((entry = zis.getNextEntry()) != null && !entry.isDirectory()) {
                ...
                String lineString = null;
                while ((lineString = br.readLine()) != null) {
                    if (lineString.contains("component") || lineString.contains("handler")) {
