...
//                        result = Integer.parseInt((String) result) / 1000;
    ...
    public static List transferTowaRcpFromDB(String towaType, byte[] ppbody) {
        ...
        for (int i = 0; i < rcpTemplates.size(); i++) {
            // 去空 
            if (rcpTemplates.get(i).getParaName() == null || rcpTemplates.get(i).getParaName().isEmpty() || "NULL".equals(rcpTemplates.get(i).getParaName())) {
...
//        int =Integer.parseInt(str, 10);
    ...
    public static void main(String args[]) {
        byte[] ppbody = (byte[]) TransferUtil.getPPBody(1, "D:\\RECIPE\\FB240X76.3 KN For NT966601_V0.txt").get(0);
        List<RecipePara> recipeParas = transferTowaRcpFromDB("TOWAPMC", ppbody);
        for (RecipePara recipePara : recipeParas) {
            if (recipePara.getParaName() != null) {
                System.out.println(recipePara.getParaName() + "---" + recipePara.getSetValue());
            }

        }
        //transferFromFile4Isecs("D:\\RECIPE\\3B-3-D-0'50-A0.rsp_V0");
    }

    //下面两个方法用于解析isecs通讯时的towa recipe
    public static Map transferFromFile4Isecs(String filePath) {
...
//                    }
                    ...
                    key = tmp[0].replaceAll("\"", "") + tmp[1].replaceAll("\"", "");
                    map.put(key, tmp[2]);
    ...
    public static List transferFromDB4Isecs(Map paraMap, String deviceType) {
        ...
        for (Map.Entry<String, String> e : entry) {
            if (paraNameList.contains(e.getKey())) {
                ...
                recipePara.setParaShotName(recipeTemplates.get(paraNameList.indexOf(e.getKey())).getParaShotName());
                recipeParaList.add(recipePara);
                // System.out.println("" + recipePara.getParaCode() + "  " + recipePara.getSetValue() + " " + recipePara.getParaName());
            }
        }
        return recipeParaList;
    }
