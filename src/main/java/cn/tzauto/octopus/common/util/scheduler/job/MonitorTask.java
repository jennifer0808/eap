            ...
            for (EquipHost equipHost : GlobalConstants.stage.equipHosts.values()) {
                ...
                try {
                    ...
                    if ("Engineer".equals(busniessMod)) {
                        UiLogUtil.appendLog2SecsTab(deviceCode, "工程模式，取消实时Check卡控！");
                        continue;
                    }
                    if (!"Y".equals(deviceInfoExt.getRecipeAutoCheckFlag())) {
//                        UiLogUtil.appendLog2SecsTab(deviceCode, "该设备未设置参数实时监控！");
                    ...
                    if (recipeTemplates == null || recipeTemplates.isEmpty()) {
                        UiLogUtil.appendLog2SecsTab(deviceCode, "该设备没有实时参数受到管控！");
                        continue;
                    }
                    ...
                    if (resultMap != null && !resultMap.isEmpty()) {//                   
                        ...
                        UiLogUtil.appendLog2SeverTab(deviceCode, "成功发送定时检测该设备的sv参数实时信息到服务端");
                        //保存实时参数信息
                        // monitorService.saveDeviceRealtimePara(deviceRealtimeParas);
                        UiLogUtil.appendLog2SecsTab(deviceCode, "获取到该设备的SV参数实时信息！");
                        ...
                        UiLogUtil.appendLog2SecsTab(deviceCode, "获取该设备的SV参数实时信息，获取失败！");
                    }
                    sqlSession.commit();
                } catch (Exception e) {
                    sqlSession.rollback();
                ...
                if (equipModel.deviceType.contains("BTU") || equipModel.deviceType.contains("HELLER") || equipModel.deviceType.contains("HTM5022")) {
                    try {
                        ...
                        if (!equipModel.ppExecName.equals("--") && !equipModel.ppExecName.equals(deviceInfoExt.getRecipeName())) {
                            UiLogUtil.appendLog2EventTab(deviceCode, "已选程序与领料程序不一致,设备被锁定！请联系ME处理！领料程序:" + deviceInfoExt.getRecipeName() + " 已选程序:" + equipModel.ppExecName);
