 ...
 * @author Wang DanFeng
 */
public class MarchFlexTRAKHost extends EquipModel {

    private static Logger logger = Logger.getLogger(MarchFlexTRAKHost.class.getName());
    private long getRecipeAndStatusTime = new Date().getTime();

    public MarchFlexTRAKHost(String devId, String remoteIpAddress, int remoteTcpPort, String deviceType, String iconPath, String equipRecipePath) {
...
//                List<String> result = iSecsHost.executeCommand("goto any");
...
//        return map;
    ...
    @Override
    public Object clone() {
        MarchFlexTRAKHost newEquip = new MarchFlexTRAKHost(deviceId, remoteIPAddress, remoteTCPPort, deviceType, iconPath, equipRecipePath);
    ...
    public List<String> getEquipAlarm() {
        // TODO: 2018/3/19 查询报警?要通过alert做报警处理
    ...
    public String getEquipStatus() {
        try {
            synchronized (iSecsHost.iSecsConnection.getSocketClient()) {
                List<String> result = iSecsHost.executeCommand("curscreen");
                if (result != null && !result.isEmpty()) {
                        // TODO: 2018/9/19  区域重新定义
                        result = iSecsHost.executeCommand("readrectcolor 164 511 165 512" );
        ...
        if (run != null && !run.isEmpty() && !run.get(0).contains("error") && !"done".equalsIgnoreCase(run.get(0))) {
            if (!run.contains("0x8")) {
                ...
                if (ready != null && !ready.isEmpty() && !ready.get(0).contains("error") && !"done".equalsIgnoreCase(ready.get(0))) {
                    if (!ready.contains("0x8")) {
                        if (ready.contains("0xff")) {
                            equipStatus = "READY";
                        } else {
                            equipStatus = "SETUP";
                        }
                        ...
                        if (error != null && !error.isEmpty() && !error.get(0).contains("error") && !"done".equalsIgnoreCase(error.get(0))) {
                            if (!error.contains("0x8")) {
                                if (error.contains("0xff")) {
                                    equipStatus = "ERROR";
                                } else {
                                    equipStatus = "SETUP";
                                }
                                ...
                                if (stop != null && !stop.isEmpty() && !stop.get(0).contains("error") && !"done".equalsIgnoreCase(stop.get(0))) {
                                    if (!stop.contains("0x8")) {
                                        if (stop.contains("0xff")) {
                                            equipStatus = "STOP";
                                        } else {
                                            equipStatus = "SETUP";
                                        }
