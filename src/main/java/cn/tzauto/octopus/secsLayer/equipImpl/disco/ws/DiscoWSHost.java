...
public class DiscoWSHost extends EquipHost {
    ...
    public DiscoWSHost(String devId, String equipmentId, String smlFileFullPath, String localIpAddress,
    ...
    protected void processS6F11EquipStatusChange(DataMsgMap data) {
        ...
        try {
            ...
            if (equipStatus.equalsIgnoreCase("run")) {
                ...
                if (checkRecipe == null) {
                    UiLogUtil.appendLog2EventTab(deviceCode, "工控上不存在: " + shortName + "！ 请确认是否已审核通过！");
                    this.holdDevice();
                    return;
                }
                if (!shortName.equals(checkRecipe.getRecipeName())) {
                    UiLogUtil.appendLog2EventTab(deviceCode, "设备使用程序： " + shortName + " ;与领料程序：" + checkRecipe.getRecipeName() + " 不一致，禁止开机，设备被锁定！请联系ME处理！");
                    this.holdDevice();
                    return;
                }
                //检查程序是否存在 GOLD
                ...
                if (goldRecipe == null) {
                    UiLogUtil.appendLog2EventTab(deviceCode, "工控上不存在： " + shortName + " 的Gold版本，无法执行开机检查，设备被锁定！请联系PE处理！");
                    this.holdDevice();
                    return;
                }
//                    startCheckEC();
...
//                realTimePara.setVisible(true);
    ...
    @Override
    public Object clone() {
        DiscoWSHost newEquip = new DiscoWSHost(deviceId, this.deviceCode,
